...
path = "C:\\Users\\79175\\Documents\\GitHub\\tir\\src\\TIRDB.db"  # Адрес БД
...
def keyboard_data_pleace(message, date_event):  # Набросок клавиатуры для записи на тренировку
    connection = create_connection(path)
    telegram_id = message.from_user.id
    registration_script = f"""
...
#@eholandbot.callback_query_handler(func=keyboard_data_pleace)
...
def cal(c):
    ...
    if not result and key:
        ...
        if result < current_date:
            ...
            date_event = result
            print(date_event)
            keyboard_data_pleace(c, date_event)
